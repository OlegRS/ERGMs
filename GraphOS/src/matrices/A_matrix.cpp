...
A_matrix& A_matrix::sample_AKS_model_with_single_link_Metropolis(const unsigned int &N_iters, prng& rnd, const double tau_1, const double mu, const double lambda, const bool &initialize_randomly, const double &temp) {
  col_vector<double> T(dim_x-1);
  T[0] = tau_1;
  for(unsigned int i=1; i<T.size(); ++i) // i==k-1
    T[i] = mu/pow(-lambda, i-1);
  
  return sample_p_star_model_with_single_link_Metropolis(N_iters, rnd, T, initialize_randomly, temp);
}
